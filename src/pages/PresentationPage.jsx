...
const specialties = [
  ...
  { name:'구강내과',       desc:'턱관절 통증·이갈이·코골이부터 입 냄새, 입안 점막 질환까지! 입·얼굴·턱의 다양한 질환을 정밀 검사로 진단하고 치료해요.', color:'#FF7043', image:'omedison.png' },
]
...
const timeline = [
  ...
  { time:'09:30', emoji:'🦷', color:'#42A5F5', title:'오전 진료 시작!', sub:'치아 구조대 출동',
    ...
    detail:'하루에 보통 20~30명의 환자를 만나요. 집중력이 중요한 시간이에요.',
    image:'/zone0.png' },
  ...
  { time:'17:30', emoji:'✅', color:'#EF5350', title:'마지막 환자 & 정리', sub:'오늘도 수고했어요!',
    ...
    detail:'하루 동안 20~30명의 치아를 지킨 진짜 영웅! 마무리까지 완벽하게요.',
    image:'/jobs/bochul.png' },
      ...
      <div style={{ flex:1, position:'relative', overflow: mob() ? 'auto' : 'hidden' }}>
        ...
        <svg viewBox="0 0 1000 520" style={{ width:'100%', height:'100%', display:'block', minWidth: mob() ? 700 : undefined }} preserveAspectRatio="xMidYMid meet">
          ...
          <g style={glow(0)} onClick={()=>onEnter(0)} onMouseEnter={()=>setHovered(0)} onMouseLeave={()=>setHovered(null)}>
            ...
            <image href={hovered===0 ? '/zone0_over.png' : '/zone0.png'} x="59" y="208" width="176" height="234" preserveAspectRatio="xMidYMid meet"
              style={{ transformOrigin:'147px 325px', transform: hovered===0 ? 'scale(1.05)' : 'scale(1)', transition:'transform 0.25s ease' }}/>
            ...
            <circle cx="77" cy="479" r="5.4" fill="none" stroke="white" strokeWidth="1.2"/>
            <line x1="77" y1="479" x2="77" y2="474.8" stroke="white" strokeWidth="1.2" strokeLinecap="round"/>
            <line x1="77" y1="479" x2="80" y2="480.8" stroke="white" strokeWidth="1.2" strokeLinecap="round"/>
            <circle cx="77" cy="479" r="0.8" fill="white"/>
            <text x="155" y="475" textAnchor="middle" fontSize="10" fill="white" fontWeight="bold">치과 의사의 하루</text>
            <text x="155" y="487" textAnchor="middle" fontSize="10" fill="rgba(255,255,255,0.85)">출근부터 퇴근까지</text>
            <text x="215" y="470" textAnchor="middle" fontSize="12" fill="white" opacity="0.75">✦</text>
            <text x="223" y="483" textAnchor="middle" fontSize="8" fill="white" opacity="0.55">✧</text>
          </g>
          ...
          <g style={glow(1)} onClick={()=>onEnter(1)} onMouseEnter={()=>setHovered(1)} onMouseLeave={()=>setHovered(null)}>
            ...
            <text x="390" y="475" textAnchor="middle" fontSize="10" fill="white" fontWeight="bold">치과에서 어떤 치료를?</text>
            <text x="390" y="487" textAnchor="middle" fontSize="10" fill="rgba(255,255,255,0.85)">치과치료의 종류</text>
          ...
          <g style={glow(3)} onClick={()=>onEnter(3)} onMouseEnter={()=>setHovered(3)} onMouseLeave={()=>setHovered(null)}>
            ...
            {hovered===3 && [
              ...
              <text key={i} x={h.x} y={h.y} fontSize={h.sz} textAnchor="middle" fill={h.c} opacity="0">
                ♥
                <animate attributeName="opacity" values="0;0.45;0.25;0" dur="2.2s" begin={h.delay} repeatCount="indefinite"/>
                <animate attributeName="y" values={`${h.y};${h.y-32}`} dur="2.2s" begin={h.delay} repeatCount="indefinite"/>
                <animate attributeName="x" values={`${h.x};${h.x+h.dx*4}`} dur="2.2s" begin={h.delay} repeatCount="indefinite"/>
              </text>
            ))}
          </g>
          ...
          <g style={glow(2)} onClick={()=>onEnter(2)} onMouseEnter={()=>setHovered(2)} onMouseLeave={()=>setHovered(null)}>
            ...
            <rect x="777.2" y="473.6" width="9.6" height="10.8" rx="1.2" fill="white"/>
            <line x1="779" y1="476" x2="786" y2="476" stroke="#F45B69" strokeWidth="1.2"/>
            <line x1="779" y1="478.4" x2="786" y2="478.4" stroke="#F45B69" strokeWidth="1.2"/>
            <line x1="779" y1="480.8" x2="786" y2="480.8" stroke="#F45B69" strokeWidth="1.2"/>
            <text x="858" y="475" textAnchor="middle" fontSize="10" fill="white" fontWeight="bold">치과의사가 되는법</text>
  ...
  return (
    <div style={{ width:'100vw', height:'100vh', display:'flex', flexDirection:'column', fontFamily:"'Noto Sans KR',sans-serif" }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, padding: isMob ? '12px 16px' : '14px 32px', background:'linear-gradient(135deg,#0A2416,#1A5C3A)', boxShadow:'0 2px 16px rgba(0,0,0,0.2)', flexShrink:0, flexWrap: isMob ? 'wrap' : 'nowrap', color:'white' }}>
        <BackBtn onClick={onBack} />
        <h1 style={{ fontSize: isMob ? '1.0rem' : '1.5rem', fontWeight:900, color:'white' }}>치과에서 어떤 <span style={{ color:'#F5C800' }}>치료를?</span></h1>
      ...
      <div style={{ flex:1, overflow:'hidden', display:'flex', flexDirection:'column' }}>
        ...
        {tab === 1 && <ToolsContent />}
        {tab === 2 && <FactsSection onBack={() => setTab(0)} />}
      </div>
    </div>
  )
}
